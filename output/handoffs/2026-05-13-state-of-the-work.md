...
*v3 additions (2026-05-12 evening, "Option A ship"): trained CEJ polyline model on pickles (88 epochs, early-stopped at epoch 58 best). Karpathy-style stratification on 200 test images showed predict_success monotonic with GT supervision density (20% / 60% / 88% / 100% by 0-2 / 3-4 / 5-7 / 8+ GT CEJ points). `max_conf >= 0.5` is a clean 100%-success filter on 66% of images. Wired the full v3 inference pipeline: `_run_cej_polyline()` stage runs after segmentation; `_build_findings_from_stages()` routes high-conf images through Family A mm math (apex-free) and emits "low_model_confidence" findings for the rest. Render layer prefers `mm_estimate` over legacy `pct` labels and shows a chairside "⚠ Manual measurement recommended for #X, #Y" badge. v3 Phase 2 deleted the apex head entirely: `get_keypoint_apex`, `_LANDMARK_DEFAULT_KP["keypoint_apex"]`, the apex branch in `_run_keypoint_passes`, and the apex-dependent `compute_bone_loss_pct` calls are all gone. Legacy keypoint pathway now computes mm directly from keypoint y-coords (no apex involved). `weights/keypoint_apex.pt` becomes unused on tenant pods. severity.py kept in-tree but its function is unused at runtime. All 142 tests green (139 prior + 3 routing tests). v0 ship state achieved.*

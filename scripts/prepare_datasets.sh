...
#
# Outputs (under data/denpar/prepared/):
#   yolo_detect/  dataset.yaml + images/{train,val,test}/*.jpg + labels/.../*.txt
#   yolo_tooth_seg/  same shape, 1 class = tooth (instance polygons)
#   yolo_bone_seg/   same shape, 1 class = bone (single polygon per image)
#   keypoints/    {train,val,test}/{images/*.jpg, annotations.json}  COCO-keypoints
#
...
PREPARED_ROOT="${DENPAR_ROOT}/prepared"
...
# YOLO detection (2 classes: single, double)
yaml = build_yolo_dataset(denpar_root, prepared / "yolo_detect", "tooth_detect")
log.info("yolo_detect dataset.yaml: %s", yaml)
...
ann = build_coco_keypoints(denpar_root, prepared / "keypoints", "cej")
log.info("keypoints train annotations: %s", ann)
PY
